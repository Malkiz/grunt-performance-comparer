(function () {
	...
	function compare(base, current, threshold, getOutputFileNameFn) {
		emitter.emit('runStarted', threshold);
		var tooSlow = {};
		var faster = {};
		...
		loopBoth(base, current, function (filename, baseFile, currentFile) {
			emitter.emit('fileStarted', filename);
			emitter.emit('suiteStarted', 'All Browsers');
			var specs = {};
			...
			loop(specs, function (fullName, specsObj) {
				...
				if (failureCount >= threshold.suites || (browserNames.length < threshold.suites && failureCount > 0)) {
					totalFailureCount++;
					suiteFailureCount++;
					emitter.emit('specFailed', failureMessage);
				} else if (skippedCount > 0) {
					emitter.emit('specSkipped', skipMessage);
				}
			});

			emitter.emit('suiteEnded', {
				'tests': suiteCount,
				'failures': suiteFailureCount,
			});

			var outputPath = getOutputFileNameFn(filename);
			emitter.emit('fileEnded', outputPath);
		...
		return {tooSlow: tooSlow, faster: faster, pathsArr: pathsArr, totalCount: totalCount, failureCount: totalFailureCount, skippedCount: totalSkippedCount};
	}
