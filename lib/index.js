(function () {
	...
	function compare(base, current, threshold, thresholdType, getOutputFileNameFn) {
		var tooSlow = {};
		var faster = {};
		...
		var thresholdManipulators = {
			ms : function(time, other) {
				return time + ((threshold || 0) / 1000);
			},
			'%' : function(time, other) {
				return time + (other / 100 * (threshold || 0));
			}
		};

		var thresholder = thresholdManipulators[thresholdType] || thresholdManipulators['ms'];
		...
		loopBoth(base, current, function(filename, baseFile, currentFile) {
			...
			loopBoth(baseFile, currentFile, function(testsuite, baseSuite, currentSuite) {
				...
				loopBoth(baseSuite.testcases, currentSuite.testcases, function(fullName, baseResult, currentResult) {
					...
					if (thresholder(baseTime, currentTime) < currentTime) {
						suiteFailureCount++;
					...
					} else if (thresholder(currentTime, currentTime) < baseTime) {
		...
		loop(data, function (filename, fileObj){
			loop(fileObj, function (testsuite, suiteObj) {
				...
				loop(suiteObj.testcases, function (fullName, result) {
					...
					joinedSuite[fullName].noPerformanceTest = (joinedSuite[fullName].noPerformanceTest || (result.noPerformanceTest == "true"));
