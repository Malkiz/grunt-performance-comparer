			...
			loopBoth(baseFile, currentFile, function(testsuite, baseSuite, currentSuite) {
				...
				loopBoth(baseSuite.testcases, currentSuite.testcases, function(fullName, baseResult, currentResult) {
					...
					var spec = suite.ele('testcase', {
						...
						classname: currentResult.classname, 
						baseTime: baseTime, 
						currentTime: currentTime
				    });

				    if (baseResult.times) {
				    	spec.att('baseTimes', baseResult.times);
				    }
				    if (currentResult.times) {
				    	spec.att('currentTimes', currentResult.times);
				    }

				    var shouldSkip = (currentResult.noPerformanceTest == "true" || 
				    	currentResult.noPerformanceTest == true ||
				    	(baseResult.std != null && baseResult.std > threshold.std) ||
					...
					if (baseTime + threshold.avg < currentTime) {
						suiteFailureCount++;
						pushData(filename, testsuite, fullName, baseTime, currentTime, tooSlow);
				    	spec.ele('failure', {type: ''}, 
				    		'Current runtime (' + currentTime + ') exceeded baseTime (' + baseTime + ') + threshold (' + threshold.avg + ')\n' +
